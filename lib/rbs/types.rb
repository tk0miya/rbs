...
module RBS
  module Types
    # Internal helpers for identity-preserving `map_type` / `map_type_name`
    # implementations. The "i-map" variants iterate without allocating
    # while every block result is `equal?` to its source element; only on
    # the first divergence do they copy the prefix and continue.
    module IMap
      module_function

      def imap_array(array)
        out = nil
        i = 0
        size = array.size
        while i < size
      ...
      def imap_hashv(hash)
        ...
        hash.each_pair do |k, v|
          ...
          if out
            out[k] = mapped
          elsif !mapped.equal?(v)
            out = {}
            hash.each_pair do |k0, v0|
              break if k0.equal?(k) || k0 == k
              out[k0] = v0
            end
            out[k] = mapped
          end
    ...
    class ClassSingleton
      ...
      def map_type_name(&block)
        ...
        new_args = IMap.imap_array(args) { |type| type.map_type_name(&block) }
      ...
      def map_type(&block)
        if block
          new_args = IMap.imap_array(args) { |type| yield type }
    ...
    class Alias
      ...
      def map_type_name(&block)
        new_name = yield(name, location, self)
        new_args = IMap.imap_array(args) { |arg| arg.map_type_name(&block) }
        return self if new_name.equal?(name) && new_args.equal?(args)
    ...
    class Tuple
      ...
      def map_type_name(&block)
        new_types = IMap.imap_array(types) { |type| type.map_type_name(&block) }
      ...
      def map_type(&block)
        if block
          new_types = IMap.imap_array(types) { |type| yield type }
          return self if new_types.equal?(types)
          Tuple.new(types: new_types, location: location)
        else
          enum_for :map_type
    ...
    class Record
      ...
      def map_type_name(&block)
        new_all_fields = IMap.imap_hashv(all_fields) do |pair|
          ty, required = pair
          new_ty = ty.map_type_name(&block)
          new_ty.equal?(ty) ? pair : [new_ty, required]
        end
        return self if new_all_fields.equal?(all_fields)
    ...
    class Union
      ...
      def map_type(&block)
        if block
          new_types = IMap.imap_array(types, &block)
          return self if new_types.equal?(types)
    ...
    class Function
      ...
      def map_type(&block)
        if block
          new_required_positionals  = IMap.imap_array(required_positionals)  { |param| param.map_type(&block) }
          new_optional_positionals  = IMap.imap_array(optional_positionals)  { |param| param.map_type(&block) }
          new_rest_positionals      = rest_positionals&.map_type(&block)
          new_trailing_positionals  = IMap.imap_array(trailing_positionals)  { |param| param.map_type(&block) }
          new_required_keywords     = IMap.imap_hashv(required_keywords)     { |param| param.map_type(&block) }
          new_optional_keywords     = IMap.imap_hashv(optional_keywords)     { |param| param.map_type(&block) }
    ...
    class Block
      ...
      def map_type(&block)
        ...
        return self if new_type.equal?(type) && new_self_type.equal?(self_type)
        Block.new(
          required: required,
          type: new_type,
    ...
    class Proc
      ...
      def map_type_name(&block)
        new_type = type.map_type_name(&block)
        new_block = self.block&.map_type { |t| t.map_type_name(&block) }
