...
module RBS
  class Environment
    ...
    def resolve_signature(resolver, table, dirs, decls, only: nil)
      ...
      decls = Types.map_with_change(decls) do |decl|
    ...
    def resolve_type_names(only: nil)
      ...
      each_rbs_source do |source|
        ...
        if decls.equal?(source.declarations)
      ...
      each_ruby_source do |source|
        decls = Types.map_with_change(source.declarations) do |decl|
          if only
    ...
    def resolve_declaration(resolver, map, decl, context:, prefix:)
      ...
      when AST::Declarations::Class
        ...
        new_super_class = decl.super_class&.then do |super_class|
          ...
          new_super_args = Types.map_with_change(super_class.args) {|type| absolute_type(resolver, map, type, context: outer_context) }
        ...
        new_members = Types.map_with_change(decl.members) do |member|
          case member
      ...
      when AST::Declarations::Module
        ...
        new_name = decl.name.with_prefix(prefix)
        new_type_params = resolve_type_params(resolver, map, decl.type_params, context: inner_context)
        new_self_types = Types.map_with_change(decl.self_types) do |module_self|
          new_self_name = absolute_type_name(resolver, map, module_self.name, context: inner_context)
          new_self_args = Types.map_with_change(module_self.args) {|type| absolute_type(resolver, map, type, context: inner_context) }
    ...
    def resolve_member(resolver, map, member, context:)
      ...
      when AST::Members::MethodDefinition
        new_overloads = Types.map_with_change(member.overloads) do |overload|
      ...
      when AST::Members::Include
        ...
        new_args = Types.map_with_change(member.args) {|type| absolute_type(resolver, map, type, context: context) }
    ...
    def resolve_type_params(resolver, map, params, context:)
      Types.map_with_change(params) do |param|
