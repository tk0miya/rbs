...
module RBS
  module AST
    class TypeParam
      ...
      def map_type(&block)
        new_upper = upper_bound_type ? yield(upper_bound_type) : nil
        new_lower = lower_bound_type ? yield(lower_bound_type) : nil
        new_default = default_type ? yield(default_type) : nil

        if new_upper.equal?(upper_bound_type) &&
           new_lower.equal?(lower_bound_type) &&
           new_default.equal?(default_type)
          return self
        end
